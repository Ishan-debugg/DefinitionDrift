...
def ensure_definitions():
    ...
    SEED = [
        ("net_revenue",      "Net revenue: SalesAmount minus ReturnAmount across all channels. Also known as revenue after returns, refunds, deductions.",
         "SELECT SUM(SalesAmount-ReturnAmount) FROM FactSales",    ["finance","revenue","returns","refunds"]),
        ("gross_sales",      "Gross sales: Total SalesAmount before any returns or deductions across all channels. Also known as total sales, gross revenue.",
         "SELECT SUM(SalesAmount) FROM FactSales UNION ALL SELECT SUM(SalesAmount) FROM FactOnlineSales",
         ["finance","revenue","sales","gross"]),
        ("return_rate",      "Return rate: percentage of units returned or sent back. ReturnQuantity divided by SalesQuantity times 100 across all channels. Also known as refund rate.",
         ...
         ["operations","returns","rate","refunds"]),
        ("gross_margin",     "Gross margin: SalesAmount minus ReturnAmount minus TotalCost in absolute dollars, store channel only. Also known as profit margin on store sales.",
         "SELECT SUM(SalesAmount-ReturnAmount)-SUM(TotalCost) FROM FactSales",
         ["finance","margin","profit","store"]),
        ("gross_margin_pct", "Gross margin percentage: (SalesAmount-ReturnAmount-TotalCost)/SalesAmount*100. Also known as margin percent on store sales.",
         "SELECT ROUND((SUM(SalesAmount-ReturnAmount)-SUM(TotalCost))*100.0/NULLIF(SUM(SalesAmount-ReturnAmount),0),2) FROM FactSales",
         ["finance","margin","percent","store"]),
        ("units_sold",       "Units sold: SalesQuantity minus ReturnQuantity, store channel only. Number of units sold in store.",
         "SELECT SUM(SalesQuantity-ReturnQuantity) FROM FactSales",
         ["operations","sales","units","store"]),
        ("online_units_sold","Online units sold: SalesQuantity minus ReturnQuantity, online channel only. Number of units sold online.",
         "SELECT SUM(SalesQuantity-ReturnQuantity) FROM FactOnlineSales",
         ["operations","online","units"]),
        ("avg_order_value",  "Average order value: Average SalesAmount per transaction, store channel only. Also known as AOV, average transaction value.",
         "SELECT ROUND(AVG(SalesAmount),2) FROM FactSales WHERE SalesAmount>0",
         ["sales","store","average","transaction"]),
    ]
